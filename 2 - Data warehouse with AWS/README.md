...
3 - Take note of the access key and secret and edit the `dwh.cfg` accordingly to include the required info<br>
4 - Run the `create_redshift_cluster.py` script to create the Redshift cluster<br>
5 - Run the `create_tables.py` script to create the staging and dimensional tables in the cluster<br>
6 - Run the `etl.py` script to perform the ETL from S3 to the staging tables, and then from staging to dimensional analytics tables<br>
7 - Open the `Redshift Query Editor` for the cluster to check the table data.<br>
<br>
